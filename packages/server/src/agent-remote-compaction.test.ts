...
test("remote compaction middleware captures, persists once, and skips non-Codex", async () => {
  ...
  const middleware = remoteCompactionMiddleware("codex", "gpt-5.6-sol", { sessionId, fetch: fetchStub as unknown as typeof fetch, requestHeaders: stubHeaders })
  ...
  let nonCodexCalls = 0
  const otherFetch = (async () => { nonCodexCalls++; return sseResponse() }) as unknown as typeof fetch
