...
const input = z.object({ file: z.string().optional(), line: z.number().int().positive().optional(), col: z.number().int().positive().optional(), symbol: z.string().optional() }).refine(x => x.symbol || (x.file && x.line && x.col), "provide symbol or file, line, and col")
...
async function run(args: z.infer<typeof input>, config: unknown, refs: boolean) {
  try { const root = workspaceFromConfig(config); const m = getCodegraph(root); const hits = args.symbol ? await m.query(args.symbol, refs) : await m.at(args.file!, args.line!, args.col!, refs); return format(hits, root) } catch (e) { return `Codegraph unavailable: ${e instanceof Error ? e.message : String(e)}` }
}
