...
export function agentPlanFor(selection: AgentSelection, sessionId?: string) {
  ...
  return {
    ...
    hasReview,
    sidekickSeats,
    sidekickConfig: effectiveSidekickConfig(sessionId),
    sidekickSeatConfigs: effectiveSidekickSeats(sessionId),
    nativeToolSearch,
...
export function buildAgent(
  ...
  return createAgent({
    ...
    systemPrompt: (repoLess ? buildRepoLessSystemPrompt : buildSystemPrompt)(plan.editToolName, plan.hasAdvisor, workspace, {
      ...
      sidekickConfig: plan.sidekickConfig,
      sidekickSeatConfigs: plan.sidekickSeatConfigs,
      agentsMd,
      repoMemory,
    }),
...
export function getAgent(
  ...
  if (!a) {
    a = buildAgent(selection, workspace, { repoLess: sessionId ? Store.repositoryScopeOf(sessionId) === "none" : false }, agentsMd, sessionId, repoMemory)
