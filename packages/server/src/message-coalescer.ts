...
export function createMessageCoalescer<T>(
  persist: (sessionId: string, event: AgentEvent) => T,
  onPersisted?: (sessionId: string, event: AgentEvent, result: T) => void,
) {
  ...
  const persistEvent = (sessionId: string, event: AgentEvent) => {
    const result = persist(sessionId, event)
    onPersisted?.(sessionId, event, result)
  }
  const flush = (sessionId: string, ev: AgentEvent, final = false) => {
    ...
    if (chunk || final) persistEvent(sessionId, { type: "message.delta", text: chunk, ...(buffer.threadId ? { threadId: buffer.threadId } : {}) })
