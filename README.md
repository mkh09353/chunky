...
- ✅ **Bounded transcript history**: `GET /api/sessions/:id/history?turns=&before=` returns a contiguous tail anchored on root user turns (default 10, clamped to 1–100), plus generation-bound cursors for v2 stream resume and older-page keyset reads. Rewritten page cursors return HTTP 409 `history-rewritten`.
