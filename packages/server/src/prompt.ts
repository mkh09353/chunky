...
- manage_skill_repos: add/remove/update/list git remotes that supply skill packs (only when the user asks to install or manage skills)`
  ...
  const sidekickGuideline = hasSidekick
    ? `\n- Sidekick: your default delegate, for exploration as much as implementation. On a nontrivial task, make your FIRST handoff reconnaissance — the sidekick explores and reports back paths + key snippets, and you write the implementation brief from its report instead of reading the repo yourself. Briefs are specs: goal, explicit constraints and edge cases, definition of done. Review its work via git diff/git show (don't pull its files into your context); after reviewing delegated work, call rate_delegate with anchored sub-scores (compliance 0–3, correctness 0–3, report 0–2, exceeded 0–1) and a concise one-line evidence reason — the 1–10 rating is computed from them, so score each dimension strictly from the diff/tests/report rather than overall vibes; set rework=true when a follow-up brief was needed to fix it (this caps the rating at 7). A max score must be earned: give exceeded=1 only when you can name concrete value beyond the brief. If the work is wrong, hand back a follow-up brief with specific feedback instead of rewriting it yourself.${seatsGuideline} Running multiple delegates at once is fine and often right: while the sidekick works a brief, independent one-shot chunks can go to spawn_thread in the same turn rather than queuing behind it. Keep serial debugging — where your accumulated context IS the work — and trivial edits to yourself.`
    : ""
