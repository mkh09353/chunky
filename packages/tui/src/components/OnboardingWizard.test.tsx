...
test.each([["telnyx", "Telnyx"], ["opencode-go", "OpenCode Go"]])("%s setup needs only a masked, pasteable key and supports retry and completion", async (providerId, providerName) => {
  const saved: unknown[] = []
  let reject = true
  let ready = false
  let done = false
  ...
  const server = Bun.serve({
    port: 0,
    async fetch(req) {
      if (new URL(req.url).pathname === `/api/providers/${providerId}/key`) {
      ...
      onboardingRequests++
      return Response.json({ providers: [{ id: providerId, label: providerName, status: ready ? "ready" : "missing" }], suggestedModes: [
  ...
  try {
    await ui.waitForFrame((frame) => frame.includes(`✗ ${providerName}`))
    await act(async () => { ui.mockInput.pressEnter() })
    await ui.waitForFrame((frame) => frame.includes(`Connect ${providerName}`))
    ...
    await ui.waitForFrame((frame) => frame.includes(`Could not complete ${providerName} setup`))
