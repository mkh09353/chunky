...
import { saveMode, setAdvisor, setSidekick, resetSidekickSeat, setOnboardedAt } from "./settings.ts"
...
export async function suggestedModes(ready: Set<string>): Promise<OnboardingSuggestion[]> {
  ...
  if (ready.has("anthropic")) {
    try { const models = await listModelsFor("anthropic"); model = models.find((m) => /fable/i.test(m.id))?.id ?? models.find((m) => /opus/i.test(m.id))?.id ?? model } catch {}
  }
  if (ready.has("codex") && ready.has("anthropic")) return [{ name: "fire", description: "Anthropic Fable leads, with Codex workers and advice.", spec: { provider: "anthropic", model, sidekick: { provider: "codex", model: "gpt-5.6-luna", effort: "xhigh" }, advisor: { provider: "codex", model: "gpt-5.6-sol" } } }]
