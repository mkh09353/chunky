...
import { applyOnboardingMode, completeOnboarding, fetchOnboarding, initiateLogin, loginStatus, saveCustomProvider, type OnboardingPayload, type OnboardingSuggestion } from "../lib/api"
...
export function OnboardingWizard({ baseUrl, onClose, onApplied }: { baseUrl: string; onClose: () => void; onApplied: () => void }) {
  ...
  const login = async (id: string) => {
    ...
    try {
      ...
      if (init.kind !== "ready") for (let until = Date.now() + 150000; Date.now() < until; ) { await new Promise((r) => setTimeout(r, 2000)); if (await loginStatus(baseUrl, id)) break }
