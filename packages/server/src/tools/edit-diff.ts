// Multi-edit core, ported verbatim from Pi's edit-diff.ts (the crown jewel of the
// lean harness). This is the DEPENDENCY-FREE subset: exact-then-fuzzy text
// matching, multi-edit application against the ORIGINAL content (not incremental),
// overlap detection, and line-ending / BOM handling. The diff-preview helpers from
// Pi (generateUnifiedPatch / generateDiffString / computeEditsDiff) are omitted on
// purpose — they need the `diff` npm package, which we don't ship, and we don't
// render diff previews server-side.
